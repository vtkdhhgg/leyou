          ...
          <v-row justify="center">
            <v-dialog v-model="dialog" persistent max-width="290">
              ...
              <v-card>
                ...
                <v-card-text>是否删除这个商品信息？</v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="green darken-1" text @click="dialog = false">是</v-btn>
                  <v-btn color="green darken-1" text @click="dialog = true">否</v-btn>
                </v-card-actions>
              </v-card>
            </v-dialog>
          </v-row>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteGoods(spuId){
        //给用户提示是否删除
        let isDelete = DeleteDialogs.data();
        console.log(isDelete);
        console.log(spuId);
        if (isDelete){
          //发起请求，删除商品信息
          this.$http.get("/item/deleteSpu?id="+spuId);
          //删除后，加载数据
          this.getDataFromServer();
        }
      },
